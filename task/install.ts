...
export default Task('install', async task => {
	...
	for (let [, path, pathText] of pathsText?.matchAll(/\n\t"([^"]*)": \{(.*?\n\t)\}/gs) ?? []) {
		...
		const fileName = `Endpoint${(pathSegments
			.map(segment => `${segment[0].toUpperCase()}${(segment
				.slice(1)
				.replaceAll(/[-_\s]+([a-zA-Z0-9])/g, (_, c: string) => c.toUpperCase())
			)}`)
			.join('')
		)}.ts`
		...
		const defFile = `
			...
			export default Endpoint('${actualPath}', '${method?.toLowerCase()}')${isNoResponse ? '.noResponse()' : ''}
		`.replaceAll(/^\t+/gm, '').trim()
