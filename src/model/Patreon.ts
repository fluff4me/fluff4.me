...
namespace Patreon {
	...
	export function translateTiers (tierIds: string[], definitions: PatreonCampaignTier[]): Quilt.Handler | undefined {
		...
		if (!selectedTiers.length)
			return quilt => quilt['view/chapter-edit/shared/form/visibility-patreon-tier/selection/none']()

		if (definitions?.length === selectedTiers.length)
			return quilt => quilt['view/chapter-edit/shared/form/visibility-patreon-tier/selection/all']()
		...
		if (definitions?.every(tier => tier.amount <= firstSelectedTier.amount || tierIds.includes(tier.tier_id))) {
			...
			return quilt => quilt['view/chapter-edit/shared/form/visibility-patreon-tier/selection/tier-and-up'](resolvedTranslation)
		}
